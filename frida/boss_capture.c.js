📦
444795 /_bosscompile/boss_capture.js
✄
...
function VM(api2) {
  ...
  this.link = function(tid2, env) {
    const entry = activeEnvs.get(tid2);
    if (entry === void 0) {
      activeEnvs.set(tid2, [env, 1]);
    } else {
      entry[1]++;
    }
  };
  this.unlink = function(tid2) {
    const entry = activeEnvs.get(tid2);
    if (entry[1] === 1) {
      activeEnvs.delete(tid2);
    } else {
      entry[1]--;
    }
  };
...
function handleMethodInvocation(jniArgs, params) {
  ...
  vm2.link(tid2, env);
  try {
    pendingCalls.add(tid2);
    let fn;
    if (fallback === null || !ignoredThreads.has(tid2)) {
      fn = handler;
    } else {
      fn = fallback;
    }
...
// _bosscompile/boss_capture.js
var HOT = ["/api/batch/requests", "search/cardlist", "zpgeek", "subReqs"];
function hot(s) {
  if (s == null) return false;
  s = "" + s;
  for (var i = 0; i < HOT.length; i++) if (s.indexOf(HOT[i]) >= 0) return true;
  return false;
}
var curUrl = {};
function tid() {
...
frida_java_bridge_default.perform(function() {
  try {
    var M = frida_java_bridge_default.use("net.bosszhipin.base.m");
    M.e.implementation = function(b, url, z) {
      curUrl[tid()] = "" + url;
      try {
        return this.e(b, url, z);
      } finally {
        delete curUrl[tid()];
      }
    };
    M.f.implementation = function(b, url, z) {
      curUrl[tid()] = "" + url;
      try {
        return this.f(b, url, z);
      } finally {
        delete curUrl[tid()];
      }
    };
    ...
    send({ tag: "ERR", where: "m", err: "" + e });
  }
  try {
    var A = frida_java_bridge_default.use("com.twl.signer.a");
    ...
    send({ tag: "ERR", where: "signer", err: "" + e });
  }
  var Buf = null;
  try {
    Buf = frida_java_bridge_default.use("okio.Buffer");
  } catch (e) {
  }
  if (Buf == null) {
    try {
      frida_java_bridge_default.enumerateLoadedClassesSync().forEach(function(n) {
        if (Buf == null && /(^|\.)okio\.Buffer$|okio\d*\.Buffer$/.test(n)) {
          try {
            Buf = frida_java_bridge_default.use(n);
          } catch (e) {
          }
        }
      });
  ...
  try {
    var ReqB = frida_java_bridge_default.use("okhttp3.Request$Builder");
          ...
          if (body != null) {
            ...
            if (Buf != null) {
              try {
                var b = Buf.$new();
                body.writeTo(b);
                var arr = b.readByteArray();
                bodyLen = arr.length;
                var hexs = "";
                for (var i = 0; i < arr.length && i < 4096; i++) {
                  var v = (arr[i] & 255).toString(16);
                  hexs += (v.length < 2 ? "0" : "") + v;
                }
                bodyHex = hexs;
              } catch (e) {
                send({ tag: "ERR", where: "body-read", err: "" + e });
              }
            }
          }
          send({ tag: "HTTP", method, url, headers: hdrs, ctype, bodyLen, bodyHex });
        }
      } catch (e) {
        send({ tag: "ERR", where: "okhttp-build", err: "" + e });
      }
      return req;
    };
    send({ tag: "INFO", msg: "okhttp3 Request$Builder.build hooked (okio=" + (Buf != null) + ")" });
  } catch (e) {
    send({ tag: "WARN", where: "okhttp", err: "" + e });
  }
  try {
    var AI = frida_java_bridge_default.use("net.bosszhipin.base.a");
